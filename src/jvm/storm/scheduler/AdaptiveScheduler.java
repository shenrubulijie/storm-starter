...
public class AdaptiveScheduler implements IScheduler {
	
	private Logger logger = Logger.getLogger(AdaptiveScheduler.class);
	private AssignmentTracker assignmentTracker = new AssignmentTracker();
	private Master master;
	@Override
	public void schedule(Topologies topologies, Cluster cluster) {
		master = Master.getInstance();
		System.out.println("Offline Scheduler?");
			...
			for (TopologyDetails topology : topologies.getTopologies()) {
				...
				if (cluster.needsScheduling(topology)) {
					...
					if (componentList != null && streamMap != null) {
						System.out.println("components: " + collectionToString(componentList));
						System.out.println("streams: " + mapToString(streamMap));
						logger.info("components: " + collectionToString(componentList));
						logger.info("streams: " + mapToString(streamMap));
						...
						System.out.println("No components and streams defined for topology " + topology.getName());
						logger.info("No components and streams defined for topology " + topology);
